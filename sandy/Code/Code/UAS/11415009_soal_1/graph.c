...
Graph addEdge(Graph G, ET Start, ET End, int Weight)
{
	...
	Edge firstChecker = s->next;
	Edge secondChecker = e->next;
	
	// Start to check existing edge
	while(firstChecker != NULL)
	{
		if(firstChecker->node == e)
		{
			printf("Edge between node-%u and node-%u has been exis\n",s->ID,e->ID);
			return G;
		}
		firstChecker = firstChecker->next;
	}
	
	while(secondChecker != NULL)
	{
		if(secondChecker->node == s)
		{
			printf("Edge between node-%u and node-%u has been exis\n",s->ID,e->ID);
			return G;
		}
		secondChecker = secondChecker->next;
	}
	//End of check
